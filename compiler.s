...
%define MAKE_BOOL(pred) \
        MAKE_BYTES_LIT T_BOOL, pred

%define MAKE_LITERAL_INT(integer) \
        MAKE_WORD_LIT T_INTEGER, integer

%define MAKE_LITERAL_FLOAT(float) \
        MAKE_WORD_LIT T_FLOAT, float

%define MAKE_LITERAL_STRING(len, str) \
		MAKE_STRING_LIT len, str

%define MAKE_LITERAL_SYMBOL(strptr) \
		MAKE_WORD_LIT T_SYMBOL, strptr
